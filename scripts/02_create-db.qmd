...
# Summarize to the count level
mutations <- mutations[, .N, by = .(Gene, SampleBarcode, effect)]
```
...
```{r}
# Write the table to the database
dbWriteTable(con, "mut", mutations)
...
```{r}
ggplot(data2, aes(x = .data[[xvar]], y = .data[[yvar]], color = .data[[colvar]])) +
